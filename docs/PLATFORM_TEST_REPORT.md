...
本文记录截至 2026-07-17 对当前 GDPP 实现执行的 macOS、Linux、Windows、Android 和 Web
验证。
...
| 编译器核心 | AppleClang C++17 | ✅ 235 / 235 测试断言，本机开发 CTest 11 / 11 | 单元与集成测试 |
...
| 独立 PCK 找不到项目扩展 | 只生成运行描述符，没有把虚拟 extension list 同步注入 PCK | ✅ 导出时注入虚拟 `.godot/extension_list.cfg`，PCK 脱离编辑器缓存仍能加载唯一项目库 |
...
- [ ] Godot 4.4、4.6、4.7 各自的三桌面完整运行差分；本轮完整矩阵只认证 4.5.2。
