...
- 动态成员 IR 必须显式区分方法和属性；生成代码必须按 GDScript 顺序且只求值一次接收者、
  参数、键和复合赋值旧值。
- 所有静态解析的脚本调用必须在类型化 IR 中携带经语义验证的参数、必选参数数和变参契约；
  C++ 后端不得通过当前文件或符号表重新猜测调用 ABI。构造器、内部类、跨脚本与 `super`
  调用统一在该契约边界完成原生存储物化，并将契约参数类型计入生成单元依赖。
...
- 所有被保留的供应商描述符和项目运行描述符必须各自唯一进入扩展注册表，但运行正确性不得依赖
  二者顺序：Godot 4.4～4.7 会从无序集合重建该文件。生成行为的注册不解析供应商类型，首次脚本
  实例化才从 ClassDB 验证并构造供应商对象，缺失或不可实例化时失败关闭。macOS arm64/x86_64
  指向同一动态库时，导出扫描事务只在验证其确为 Universal 2 后临时归一化为 `universal`；成品
  写入供应商原始描述符字节，源工程描述符在成功、失败和下次启动恢复路径中都必须还原。
