...
- 分离 Godot 分析器对 `as` 的宽松兼容规则与本地存储的不变规则：允许分析的参数化转换不会被
  误当作可直接赋值，静态已知缺失或不匹配元数据的路径在生成前失败关闭；
...
从动态 `Variant` 恢复强类型容器时，后端不依赖 godot-cpp 的隐式转换构造器，而是调用统一的
runtime storage guard，逐项比较 Array 的元素元数据或 Dictionary 的键值元数据。只有完全一致的
Godot `Variant::Type`、对象类名与脚本约束才能进入 `TypedArray`/`TypedDictionary` 原生存储；
该检查与编译期存储矩阵共享同一类型描述，避免语义分析与 C++ 模板各自猜测可转换性。
