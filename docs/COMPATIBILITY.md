...
| 最低系统构建契约 | Windows 10、macOS 11.0（arm64）、Ubuntu 22.04/glibc 2.35、Android 9/API 28、iOS 16.0；Web 无固定版本下限；编译参数、SDK schema 6 / runtime ABI 7 清单与 CI 使用同一基线，导出测试不重复覆盖模板的最低系统版本 | ✅ 规则与失败关闭测试 |
...
| 统一整数数值契约 | ✅ | header-only `numeric` 模块是前端、HIR、runtime 与生成代码的唯一 64 位整数语义来源，并作为带 SHA-256 的 SDK schema 6 / runtime ABI 7 输入分发 |
