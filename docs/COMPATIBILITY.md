...
| 最低系统构建契约 | Windows 10、macOS 11.0（arm64）、Ubuntu 22.04/glibc 2.35、Android 9/API 28、iOS 16.0；Web 无固定版本下限；编译参数、SDK schema 5 / runtime ABI 4 清单与 CI 使用同一基线，导出测试不重复覆盖模板的最低系统版本 | ✅ 规则与失败关闭测试 |
...
| `var`、`const`、`:=`、静态字段 | ✅ | 字段和局部声明、类型推断、常量写保护和 C++17 静态字段已接通；静态初始化器和静态访问器不能读取实例状态，带 getter/setter 的静态字段仍通过静态访问器而非绕过 backing 规则 |
...
| 未约束容器的运行时转换 | 🟡 | 字面量、`Variant` 和原生调用边界已生成受检强类型包装；静态类型为普通 `Array`/`Dictionary` 的变量在运行时携带或缺失类型元数据时，转换失败的报错位置和函数中止时序仍需与 GDScript VM 完整差分 |
...
| 语言辅助函数 | `range`、`len`、`load`、`preload`、`convert`、`type_exists`、`char`、`ord`、`Color8`、`is_instance_of` | 集中签名、类型检查、常量分类和 runtime lowering；常量工具已有真实 GDScript/AOT 差分 | 🟡 其余非恒定语言函数待实现 |
...
| 统一整数数值契约 | ✅ | header-only `numeric` 模块是前端、HIR、runtime 与生成代码的唯一 64 位整数语义来源，并作为带 SHA-256 的 SDK schema 5 / runtime ABI 4 输入分发 |
