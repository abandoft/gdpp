...
| 算术、比较、位运算、移位、幂 | ✅ | 由版本化内建运算符表校验；整数加减乘、取负、位运算、移位及非零除余在常量、优化、静态 C++ 与动态 Variant 路径共用模 2^64 契约。Godot 4.4.1 x86_64 解释器自身会在 `INT64_MIN / -1` 及对应取余处触发 SIGFPE，因此 4.4 只与原版 GDScript 差分其余安全子集，AOT 仍在所有目标独立验证完整边界；除零的当前函数中止时序也尚未认证 |
