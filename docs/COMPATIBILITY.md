...
| 静态迭代语义门禁 | `IterationPlan` 贯穿语义、HIR、verifier 和后端；String 按 Unicode 码点快照，Array/PackedArray 观察循环内追加，Dictionary 返回精确键类型并保留动态增长协议；Godot 4.7 GDS oracle 与 Godot 4.5.2 GDScript/AOT 差分一致 | ✅ 本轮实机 |
...
| `for`、显式迭代变量类型 | 🟡 | int/range、String、Array、Dictionary、动态协议和十种 PackedArray 均有显式语义策略；String 使用 Unicode 码点快照，Array/PackedArray 同步快速路径零 Variant 迭代器且观察循环内增长，Dictionary 精确推断 key、下标精确推断 value。TypedArray/TypedDictionary 直接复用静态快速路径；异步专用循环和全部数学类型仍待完成 |
...
- [x] P1：为同步 String、Array、Dictionary 建立显式迭代策略；Unicode 字符、实时容器增长、
  ...
  GDScript/AOT 差分。
- [ ] P1：实现异步续体中的 PackedArray/静态容器专用循环、数学类型迭代和真正的类型化容器 ABI。
