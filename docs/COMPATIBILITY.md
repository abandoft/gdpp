...
| `for`、显式迭代变量类型 | 🟡 | int/range、String、Array、Dictionary、动态协议和十种 PackedArray 均有显式语义策略；String 使用 Unicode 码点快照，Array/PackedArray 同步快速路径零 Variant 迭代器且观察循环内增长，Dictionary 精确推断 key、下标精确推断 value。异步专用循环、真正的 TypedArray/TypedDictionary ABI 和全部数学类型仍待完成 |
