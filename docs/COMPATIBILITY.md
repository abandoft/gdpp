...
| 最低系统构建契约 | Windows 10、macOS 11.0（arm64）、Ubuntu 22.04/glibc 2.35、Android 9/API 28、iOS 16.0；Web 无固定版本下限；编译参数、SDK schema 7 / runtime ABI 8 清单与 CI 使用同一基线，C++17、异常关闭、MSVC 静态 CRT、Android `c++_shared` 和嵌套 CMake 工具链均失败关闭校验 | ✅ 规则与失败关闭测试 |
...
| 统一整数数值契约 | ✅ | header-only `numeric` 模块是前端、HIR、runtime 与生成代码的唯一 64 位整数语义来源，并作为带 SHA-256 的 SDK schema 7 / runtime ABI 8 输入分发 |
