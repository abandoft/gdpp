...
constexpr std::uint32_t rotate_right(std::uint32_t value, std::uint32_t count) noexcept {
...
std::string sha256(std::string_view input) {
    ...
    for (std::size_t offset = 0; offset < message.size(); offset += 64) {
        ...
        for (std::size_t index = 16; index < words.size(); ++index) {
            const auto first = rotate_right(words[index - 15], 7U) ^
                               rotate_right(words[index - 15], 18U) ^ (words[index - 15] >> 3U);
            const auto second = rotate_right(words[index - 2], 17U) ^
        ...
        for (std::size_t index = 0; index < words.size(); ++index) {
            const auto sum_one = rotate_right(e, 6U) ^ rotate_right(e, 11U) ^ rotate_right(e, 25U);
            const auto choice = (e & f) ^ ((~e) & g);
            const auto temporary_one = h + sum_one + choice + round_constants[index] + words[index];
            const auto sum_zero = rotate_right(a, 2U) ^ rotate_right(a, 13U) ^ rotate_right(a, 22U);
